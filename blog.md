...
Schematic:
    In the schematic, I set up the basics for how the keyboard functions, outlining serial communication between the halves, setting up matricies, and i2c communication for the displays. Each key also has it's own diode to prevent ghost taps and other crosstalk. I chose to use a pair of raspberry picos because of the amount of copycat footprints that could be used for drop in replacement boards.
...
Case:
    I went with a really simple and open layout for 2 reasons, I like the sound better, and I did not have an easy way to prototype test tolerences and fit. It was originally designed to be used with heatset inserts, but it ended up being easier to drive the screws directly into the plastic. The plate was made with ai03 plate-gen, but the mounts for the stabs are the wrong kind so I had to dremel away some material to make them fit.
...
Build:
    I uh, didn't have any microcontroller headers, and broke one of the hotswap sockets that I only had one spare for so, it went about as good as it could go. It took about 9 hours of work to put it all together and get firmware tested and bug fixed.